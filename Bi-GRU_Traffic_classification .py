...
def Bi-GRU_Traffic_classification(root_normalized_dir,net_parameters,model_params,batchsize,worker_id ):
    ...
    for path in os.listdir(root_normalized_dir):
        ...
        model = Bi-GRU_build_model(model_params)
        model.compile(loss=LOSS_FUNCTION,
                      optimizer=OPTIMIZER,
                      metrics=METRICS,batch_size=batchsize  # Add the batch size here)
        model.summary()
        
         # Send local weights to the Spark master
        weights_vector = Vectors.dense(local_weights)
        weights_df = spark.createDataFrame([(worker_id, weights_vector)], ["worker_id", "weights"])
        weights_df.show()
        print("this is running Bi-GRU model: ")
    ...
    model.save(save_model_weights_dir + 'model_architecture_biGRU.h5')
    saved_models.append('model_architecture_biGRU.h5') 
    # save model weights
    model.save_weights(save_model_weights_dir + 'model_weights_biGRU.h5')      
    saved_weights.append('model_weights_biGRU.h5')
    # Get the output of the last connected layer
    last_dense_output = model.layers[-len(DENSE_LAYER)].output   
