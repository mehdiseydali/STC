...
def load_pcap_datatype(file_name_dict):
    ...
    load_layer("tls")
    load_layer("ssl")
    modified_files = []  # List to store paths of modified files 
    ...
    for k,v in file_name_dict.items():
        ...
        for f in files:
            ...
            for packet in packets:
                # Assign the category label as an optional byte to each packet
                packet[Raw].load += bytes(v)
            # Save all modified packets back to the original file
            modified_files.append(path)
        wrpcap(os.path.join(path,f), packets) 
    return modified_files, file_name_dict    